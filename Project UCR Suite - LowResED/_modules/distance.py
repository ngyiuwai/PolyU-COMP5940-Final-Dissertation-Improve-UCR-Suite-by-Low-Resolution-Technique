...
Functions:  dynamicTimeWraping (normQuery, seq, seqMean, seqStd, order, bestSoFar) --> float
            dynamicTimeWraping_true (normQuery, seq, seqMean, seqStd, order, bestSoFar) --> float    // VERY VERY SLOW!!!
            squareDTW (query, rawSeq, rawSeqMean, rawSeqStd, order, bestSoFar, constrain) --> float
            LB_Kim() --> (squareDistance, cumLB)
            LB_Keogh() --> squareDistance
"""
...
def LB_LowResED(query: list, seq: list, blockLen: int, order: list, bestSoFar: float):
    """
    This function is for calculating the LB_LowResED
    If the square distance is greater than "best-so-far" square distance, we should assume that distance is infinity.
    IMPORTANT:  "Reordering Early Abandoning" technique of UCR Suite is applied here. We assume input sequence and query are reordered.
    ================================================================
    Input:
      query:    z-normalized low resolution queries:  [[max norm query], [min norm query]]
      seq:      z-normalized low resolution subsequences:  [[max norm subseq], [min norm subseq]]
      order:    The descending order of query's absolute z-normalized value.
      bestSoFar:  stop calculating square ED if the output is greater than best-so-far. Best-so-far is a squared ED too.
    Output:
      squareDistance: float
    ...
    for i in order:
        if (i == len(seq[0]) - 1):
            ...
            # Shifting window
            d = max(
                query[1][i] - max(seq[0][i], seq[0][i+1]),
                min(seq[1][i], seq[1][i+1]) - query[0][i],
                0
            )
        distance = distance + d * d * blockLen
        if (distance > bestSoFar):
            distance = float('inf')
            break
    return distance
...
def dynamicTimeWraping(query: list, seq: list, seqMean: float, seqStd: float, cumLB: list, scBand: int, bestSoFar: float):
    ...
    ++++++ Suggest to run DTW in C in actual implementaion +++
    ++++++ Please refer to function "dynamicTimeWraping_true()" for real DTW in Python, which is very very slow. ++++++
    """
...
def LB_Keogh(queryUp: list, queryLow: list, seq: list, seqMean: float, seqStd: float, order: list, cumLB: list, bestSoFar: float):
    """
    This function is for calculating the LB_Keogh between two sequences.
    If the square distance is greater than "best-so-far" square distance, we should assume that distance is infinity.
    We assume that  envolop is built on query, but not subsequences.
    LB_Keogh is simply a case based Euclidean distance.
    - Case 1: Subsequences is within MBR envolop -> Distance = 0
    - Case 2: Subsequences is above  MBR envolop -> Distance = dist(point in Subsequences, point in MBR envolop upper bound)
    - Case 3: Subsequences is below  MBR envolop -> Distance = dist(point in Subsequences, point in MBR envolop lower bound)
    IMPORTANT:  "Early Abandoning Z-Normalization" technique of UCR Suite is applied here.
                "Reordering Early Abandoning" technique of UCR Suite is applied here. We assume input sequence and query are reordered.
    ================================================================
    Input:
      queryUp:  z-normalized query upper bound.
      queryLow: z-normalized query lower bound.
      seq:      Non-normalized subsequence.
      seqMean:  Mean of sequence, which is for z-normalization.
      seqStd:   Standard derivation of sequence, which is for z-normalization.
      cumLB:    Lower bound distance of each points calculated. Initially empty
